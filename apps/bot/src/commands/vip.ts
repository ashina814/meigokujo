    ...
    .setDescription(
      active
        ? [
            ...
            `${bar(left, days, 20)}`,
            "```",
            `期限まで **${left}日**  ／  会費 ${fmtEther(price).replace(" Ld", "Ld")}／${days}日`,
          ].join("\n")
...
export async function handleVipButton(
  ...
  const embed = new EmbedBuilder()
    ...
    .setFooter({ text: `残り ${services.vip.daysLeft(uid)}日 · 所持 ${fmtEther(services.chips.balanceOf(uid)).replace(" Ld", "Ld")}` });
