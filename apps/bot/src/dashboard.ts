...
export function buildDashboardEmbed(services: Services): EmbedBuilder {
  ...
  const integrity = services.casinoIntegrity.run();
  ...
  const casinoField = [
    ...
    integrity.ok ? "検算A〜D: 正常" : `⚠️ 検算NG: ${failedChecks.map((c) => `${c.id}(${c.name})`).join(" / ")}`,
  ...
  if (!integrity.ok) alerts.push(`賭場の検算NG ${failedChecks.map((c) => c.id).join("")}`);
