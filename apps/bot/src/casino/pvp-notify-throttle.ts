...
export function takePvpNotifyRoleIds(challengerId: string, roleIds: string[], now = Date.now()): string[] {
  ...
  if (current.cooldownUntil > 0 || (current.lastNotifiedAt > 0 && now - current.lastNotifiedAt >= PVP_NOTIFY_COOLDOWN_MS)) {
  ...
  if (nextCount >= PVP_NOTIFY_BURST_LIMIT) {
    states.set(challengerId, { burstCount: 0, cooldownUntil: now + PVP_NOTIFY_COOLDOWN_MS, lastNotifiedAt: now });
  } else {
