...
describe("賭場の常設パネルを用途別に分離する", () => {
  it("ひとり遊び・みんなで勝負・競馬・板を独立した panel kind として設置できる", () => {
    const kinds = srcOf("../src/commands/panel-kinds.ts");
    expect(kinds).toContain('{ key: "casino_games", label: "賭場 · ひとり遊び", installable: true }');
  ...
  it("旧 casino_games は混合パネルではなく、ひとり遊び専用 renderer を使う", () => {
...
describe("みんなで勝負の募集ロール通知", () => {
  ...
  it("公開募集時に設定ロールを取得し、投稿 payload の allowedMentions を限定する", () => {
