...
### 権限
- **補助運営ロールへ金銭操作 API を付与しない（I9）。** 接客・進行・開催補助などを目的とする
  非管理ロールに、返金・残高調整・配当・house 投入・強制精算を持たせない。資金操作権限は
  役職追加とは独立した金融権限として設計しレビューする。
- 資金裁定と人物処分を分離する（I10）。
- **賭博場従業員・永続対人順位卓・異議裁定は 2026-08-16 に廃止した。** 役職、ロールスロット
  `roles:casino_employee`、`/賭場運営` `/賭場証拠` `/casino-arbitration`、
  `RankedTables` 系 core、S11 永続卓復旧のいずれも存在しない。**復活させないこと。**
  旧設計は仕様書から削除済みで、Git タグ `archive/casino-ranked-before-retirement-20260816`
  にのみ残る。そのタグの内容を根拠に実装しない。
