...
describe("お守り裁定シミュレーション（buy-if-not-armed 戦略を正確に再現）", () => {
  ...
  it("丁半 × 保険符（bet=10,000, 50,000回）: 実効RTP < 100%", { timeout: 60_000 }, () => {
    const ctx = bigSetup();
  ...
  it("丁半 × 庇護の札（bet=50,000, 50,000回）: 実効RTP < 100%", { timeout: 60_000 }, () => {
    const ctx = bigSetup();
    const rtp = simulate(ctx, "higo", "loss", 50_000, 0.5, 1.94, 12_000, 222);
    expect(rtp).toBeLessThan(1.0);
  });

  it("クラッシュ1.5倍 × 保険符（bet=20,000, 50,000回）: 実効RTP < 100%", { timeout: 60_000 }, () => {
    const ctx = bigSetup();
    const rtp = simulate(ctx, "hoken", "loss", 20_000, 0.96 / 1.5, 1.5, 12_000, 333);
    expect(rtp).toBeLessThan(1.0);
  });

  it("ルーレット赤 × 保険符（bet=30,000, 50,000回）: 実効RTP < 100%", { timeout: 60_000 }, () => {
    const ctx = bigSetup();
    const rtp = simulate(ctx, "hoken", "loss", 30_000, 18 / 37, 2, 12_000, 444);
    expect(rtp).toBeLessThan(1.0);
  });

  it("丁半 × 福のお守り（bet=40,000, 50,000回）: 実効RTP < 100%", { timeout: 60_000 }, () => {
    const ctx = bigSetup();
