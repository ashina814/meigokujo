...
function expectOpeningLocked(ctx: ReturnType<typeof fundedReset>, key: string): void {
  let error: unknown;
  try { ctx.chips.deposit("alice", 10, key); } catch (caught) { error = caught; }
...
describe("PR12 開業初期化 preflight", () => {
  ...
  it("旧制度清算・新制度出資が正本の賭博場部署だけを通り、sys:dept:casino を作らない", async () => {
    ...
    const txById = (id: number) => db.prepare("SELECT from_account, to_account, amount FROM transactions WHERE id=?").get(id) as { from_account: string; to_account: string; amount: number };
  ...
  it("利用者の通常Landを変えず、変わってしまう場合は適用ごとROLLBACKする", async () => {
    ...
    const tampering = {
      backup: { backup: async () => { second.db.prepare("UPDATE balances SET amount = amount - 1 WHERE account_id = 'user:alice'").run(); return { sqliteSha256: "a".repeat(64), csv: [], createdAt: 1 }; } },
      discord: { disableLegacyCasino: async () => undefined },
    };
    await expect(second.reset.apply({ configuration: config, planHash: secondPlan.planHash, actorId: "admin:a", ...tampering })).rejects.toThrow("must not change player Land");
