...
function setup(options: { sharedChipTx?: boolean; requireOpeningV1?: boolean } = {}) {
  ...
  const chips = new ChipLedger(db, ledger, new EventLog(db), {
    chipTx,
    requireOpeningV1: options.requireOpeningV1,
  });
  for (const userId of ["a", "b"]) {
    ledger.ensureAccount(`user:${userId}`, "user");
    ledger.transfer({
      from: TREASURY, to: `user:${userId}`, amount: 1_000_000, type: "initial", actor: "test", idempotencyKey: `fund:${userId}`,
    });
  }
  return { db, ledger, chips, chipTx: chips.chipTx };
}

function count(ctx: ReturnType<typeof setup>, table: string): number {
  return (ctx.db.prepare(`SELECT COUNT(*) AS n FROM ${table}`).get() as { n: number }).n;
}
...
describe("ChipLedger", () => {
  ...
  it("預入・返還は常に1:1で、準備口座は発行済みチップを100%裏付ける", () => {
    const deposit = ctx.chips.deposit("a", 12_345, "deposit:a");
    expect(deposit).toEqual({ input: 12_345, output: 12_345, burned: 0 });
    expect(ctx.chips.balanceOf("a")).toBe(12_345);
    expect(ctx.chips.pool()).toBe(12_345);
    expect(ctx.ledger.balanceOf(ETHER_ESCROW)).toBe(12_345); // 単体構築では移行互換を検証する

    const redeem = ctx.chips.redeem("a", 2_345, "redeem:a");
    expect(redeem).toEqual({ input: 2_345, output: 2_345, burned: 0 });
  ...
  it("Land取引が重複した場合はチップ発行までロールバックする", () => {
    ctx.ledger.transfer({
      from: "user:a", to: TREASURY, amount: 1, type: "adjust", actor: "test", approvedBy: "test", idempotencyKey: "collision",
    });
  ...
  it("production構築ではopening_v1前の全資金操作を拒否し、旧残高と台帳を変えない", () => {
    ...
    locked.ledger.transfer({
      from: TREASURY, to: ETHER_ESCROW, amount: 10_000, type: "adjust", actor: "test", approvedBy: "test", idempotencyKey: "legacy:reserve",
    });
    locked.ledger.transfer({
      from: TREASURY, to: department, amount: 1_000, type: "adjust", actor: "test", approvedBy: "test", idempotencyKey: "legacy:department",
    });
    ...
    const operations: Array<() => unknown> = [
      ...
      () => locked.chips.runGroup({ groupKey: "locked:game", kind: "solo_game", actorId: "a" }, () =>
        locked.chips.transfer("legacy-user", HOUSE_HOLDER, 100, { reason: "賭け金" }),
      ),
    ];
  ...
  it("opening_v1確定後だけproduction構築の1:1操作を解放する", () => {
    ...
    opened.chipTx.captureOpening("opening_v1", [], {
      poolLand: opened.ledger.balanceOf(CHIP_ESCROW),
      fromLedgerTxId: opened.ledger.lastTransactionId(),
    });

    const deposit = opened.chips.deposit("a", 1_000, "after:deposit");
    expect(deposit).toEqual({ input: 1_000, output: 1_000, burned: 0 });
    expect(opened.ledger.balanceOf(CHIP_ESCROW)).toBe(1_000);
    const redeem = opened.chips.redeem("a", 400, "after:redeem");
    expect(redeem).toEqual({ input: 400, output: 400, burned: 0 });
