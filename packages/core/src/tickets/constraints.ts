...
export interface TicketOpenUniquenessMigrationResult {
  deletedLegacyTickets: number;
}
 ...
 * - panel_id がない旧式チケット行は削除する
 * - ticket_panels（受付パネル設定）は一切変更しない
 */
export function ensureTicketOpenUniqueness(
  db: Database.Database,
): TicketOpenUniquenessMigrationResult {
  const migrate = db.transaction(() => {
    // 旧式チケットは現行パネルと安全に対応付けられないため削除する。
    // 受付パネル設定は別テーブルなので、この処理では一切変更されない。
    const deletedLegacyTickets = db
      .prepare("DELETE FROM tickets WHERE panel_id IS NULL")
      .run().changes;
