 ...
 *
 * ■ 除外している領域
 *   confession_*（トートの耳）は完全匿名が前提の機能。称号にするとプロフィール上で
 *   「匿名相談を使った」ことが露見するため、意図的に一切扱わない。
 *   チケット(ticket_*)も相談導線なので同様に扱わない。
 *
 * ■ 名前について
...
const KIZUNA: TitleRule[] = [
  ...
  { key: "mitsugetsu", category: "kizuna", name: "月下氷人", emoji: "🌸", desc: "蜜月の縁を結んだ", check: (h) => h.asActor("recruit_matched") >= 1 },
  { key: "oborozuki", category: "kizuna", name: "朧月の客", emoji: "🌘", desc: "朧月の招きに応じた", check: (h) => h.asActor("oborozuki_invite_accepted") >= 1 },
];
...
const TOKI: TitleRule[] = [
  ...
  { key: "night_1", category: "toki", name: "丑三つの徒", emoji: "🌙", desc: "深夜2時台〜4時台に浮上していた", check: (h) => h.vc.deepNightDays >= 1 },
  { key: "night_30", category: "toki", name: "夜を統べる者", emoji: "🦉", desc: "30日 深夜帯に浮上していた", check: (h) => h.vc.deepNightDays >= 30 },
  { key: "night_100", category: "toki", name: "常闇の住人", emoji: "🌑", desc: "100日 深夜帯に浮上していた", check: (h) => h.vc.deepNightDays >= 100 },
  { key: "dawn_1", category: "toki", name: "夜明けを見た魂", emoji: "🌅", desc: "朝5時〜6時台まで浮上し続けた", check: (h) => h.vc.dawnSessions >= 1 },
  { key: "dawn_10", category: "toki", name: "朝日の常連", emoji: "☀️", desc: "10回 朝まで浮上し続けた", check: (h) => h.vc.dawnSessions >= 10 },
  { key: "dawn_50", category: "toki", name: "陽を厭わぬ亡霊", emoji: "🔆", desc: "50回 朝まで浮上し続けた", secret: true, check: (h) => h.vc.dawnSessions >= 50 },
...
const BAKUCHI: TitleRule[] = [
  ...
  { key: "jackpot_5", category: "bakuchi", name: "天に愛された男", emoji: "🌠", desc: "ジャックポットを5度 射止めた", secret: true, check: (h) => h.asActor("casino_jackpot") >= 5 },
  ...
  { key: "bigwin_2m", category: "bakuchi", name: "賭場を揺らした男", emoji: "🌋", desc: "一度の勝負で200万エテルを掴んだ", secret: true, check: (h) => h.casinoStat("biggest_win") >= 2_000_000 },
  { key: "games_half", category: "bakuchi", name: "遊び人", emoji: "🎲", desc: "賭場の遊技を5種類 遊んだ", check: (h) => h.distinctCasinoGames() >= 5 },
  { key: "games_all", category: "bakuchi", name: "賭場を知る者", emoji: "🃏", desc: "賭場の遊技を9種類 遊んだ", check: (h) => h.distinctCasinoGames() >= 9 },
...
const KANE: TitleRule[] = [
  ...
  { key: "tip_sum_100k", category: "kane", name: "散財家", emoji: "💸", desc: "投げ銭の総額が10万Ldを超えた", check: (h) => h.txSum("tip") >= 100_000 },
  { key: "tip_sum_1m", category: "kane", name: "浪費の美学", emoji: "🥂", desc: "投げ銭の総額が100万Ldを超えた", check: (h) => h.txSum("tip") >= 1_000_000 },
  ...
  { key: "dept_first", category: "kane", name: "部署の一員", emoji: "🏛", desc: "部署の金庫を動かした", check: (h) => h.txCount("dept_in") + h.txCount("dept_out") >= 1 },
...
const SHIRO: TitleRule[] = [
  ...
  { key: "room_all_kinds", category: "shiro", name: "城の間取りを識る者", emoji: "🗝", desc: "宿・蜜月・朧月・ゲーム部屋の全てを開いた", check: (h) => h.roomsOpened("normal") >= 1 && h.roomsOpened("mitsugetsu") >= 1 && h.roomsOpened("oborozuki") >= 1 && h.roomsOpened("game") >= 1 },
...
const KIWAMI: TitleRule[] = [
  { key: "all_economy", category: "kiwami", name: "城を遊び尽くす者", emoji: "🏆", desc: "送金・投げ銭・両替・株・板・商館の全てに触れた", check: (h) => h.txCount("transfer") >= 1 && h.txCount("tip") >= 1 && h.asActor("ether_buy") >= 1 && h.asActor("stock_buy") >= 1 && h.asActor("market_bet") >= 1 && h.shopPurchases() >= 1 },
  { key: "explorer_10", category: "kiwami", name: "城を歩く者", emoji: "🧭", desc: "10種類のVCに浮上した", check: (h) => h.vc.distinctChannels >= 10 },
  ...
  { key: "solo_watch", category: "kiwami", name: "誰も来ぬ夜", emoji: "🪑", desc: "延べ同席時間より、独りの浮上時間が長い（50時間以上 浮上した上で）", secret: true, check: (h) => h.vc.totalSeconds >= 50 * HOUR && h.vc.totalSeconds - h.companions.totalSeconds > h.companions.totalSeconds },
...
const DAN: TitleRule[] = [
  ...
  ...dan("dan_tip", "投げ銭", "🌹", "投げ銭を通算", [1, 10, 50, 200], (h) => h.txCount("tip")),
  ...dan("dan_room", "宿の主", "🛏", "部屋を通算", [1, 5, 20, 50], (h) => h.roomsOpened()),
