...
const THEME_15: ManualReadinessEntry[] = [
  ...
  },
  {
    ...
    specializedResolvers: ["computeInviteRootedSafe"],
    missingCapabilities: [],
    evidence: [{ file: INVITE_ROOTED_FILE, symbol: "InviteRootedSafeProfile.nextGenerationEntryDayOffsets" }],
    notes: "confirmed next-generation edgeのoccurrenceをchildのimmutable ghosted entryへanchorし、branch entry基準のJST day offsetを同じanonymous profileへ保持。後段はactivity dayOffset < child offsetだけを使いroot-before-childをfail-closedに判定でき、rooted thresholdは未固定。late credited_atでhistoryを移動させず、child anchor不明件数も別保持。",
    blockerKinds: ["none"],
    optimizationRisk: "HIGH",
  },
  {
    ...
    notes: "profile 1件=distinct direct branch 1本。A→X,Y,Zは1 profile、A→X / B→Yは2 profilesなので、child総数とindependent rooted branch数を混同しない。各profile内のactivity/child entry offsetsからroot-before-childを満たすprofileだけを数えられ、cycle/selfは除外。",
    blockerKinds: ["none"],
    optimizationRisk: "HIGH",
  },
  {
