...
const THEME_07: ManualReadinessEntry[] = [
  {
    ...
    notes: "distinctCoPresentUsersが直接この意味を表現する。",
    blockerKinds: ["none"],
    optimizationRisk: "LOW",
  },
  {
    ...
    missingCapabilities: [],
    evidence: [{ file: VC_SOURCES_FILE, symbol: "VcSocialSafeSourcePayload.distinctCoPresentUsers" }],
    notes: "同じdistinctCoPresentUsersをより長いscope windowで評価すれば表現できる（新sourceは不要、閾値/window設計の話）。",
    blockerKinds: ["none"],
    optimizationRisk: "LOW",
  },
  {
    ...
    missingCapabilities: [],
    evidence: [{ file: VC_SOURCES_FILE, symbol: "VcSocialSafeSourcePayload.distinctCoPresentUsers" }],
    notes: "No.23と同じsource、より長いwindow/高いthreshold。",
    blockerKinds: ["none"],
    optimizationRisk: "LOW",
  },
  {
    ...
    missingCapabilities: [],
    evidence: [{ file: VC_SOURCES_FILE, symbol: "VcSocialSafeSourcePayload.distinctCoPresentUsers" }],
    notes: "No.22-24と同一source系列の最高tier。",
    blockerKinds: ["none"],
    optimizationRisk: "LOW",
  },
  {
...
const THEME_13: ManualReadinessEntry[] = [
  {
    ...
    notes: "kind:\"tip\"のfacts配列の初回出現＝初回成立。",
    blockerKinds: ["none"],
...
const THEME_14: ManualReadinessEntry[] = [
  {
    ...
    notes: "activityDays配列の初回出現＝初回成立。",
    blockerKinds: ["none"],
    optimizationRisk: "MANAGED",
  },
  {
    ...
    notes: "activityDays[].activityKeyからruleがdistinct game family数を数えられる。",
    blockerKinds: ["none"],
    optimizationRisk: "MANAGED",
  },
  {
    ...
    missingCapabilities: [],
    evidence: [{ file: VC_SOURCES_FILE, symbol: "CasinoActivityDaysSourcePayload.activityDays" }],
    notes: "同じ配列からdistinct family × distinct dayの両方を導出できる。",
    blockerKinds: ["none"],
    optimizationRisk: "MANAGED",
  },
  {
    ...
    evidence: [{ file: "packages/core/src/casino/participation-history.ts", symbol: "CASINO_ACTIVITY_KEYS" }],
    notes: "「core」に含めるfamily一覧そのものが未定義（本PRでmanifestを作らない、§17）。",
...
const THEME_16: ManualReadinessEntry[] = [
  {
    ...
    specializedResolvers: [],
    missingCapabilities: [],
    evidence: [{ file: VC_SOURCES_FILE, symbol: "PublicEventParticipationsSourcePayload.participations" }],
    notes: "participations配列に1件でもあれば成立——orderable:falseはNth順序保証に影響するだけで存在判定には影響しない。",
    blockerKinds: ["none"],
    optimizationRisk: "MANAGED",
  },
  {
    ...
    missingCapabilities: [],
    evidence: [{ file: VC_SOURCES_FILE, symbol: "PublicEventParticipationsSourcePayload.participations" }],
    notes: "participations[].eventKeyからdistinct event数を算出できる。",
    blockerKinds: ["none"],
    optimizationRisk: "MANAGED",
  },
  {
    ...
    usableSources: [],
    specializedResolvers: [],
    missingCapabilities: ["実event_dateのsafe payload露出"],
    evidence: [{ file: EVENT_SERVICE_FILE, symbol: "public_events.event_date (excluded from safe payload)" }],
    notes: "recordedAtはroster確定時刻であって実event日ではない——「離れた暦期間」の判定に使うと不正確。event_date自体は正本にあるがtitle payloadへ渡していない。",
    blockerKinds: ["missing_persisted_source"],
    optimizationRisk: "MANAGED",
  },
  {
