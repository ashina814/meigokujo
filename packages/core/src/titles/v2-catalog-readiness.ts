...
const THEME_02: ManualReadinessEntry[] = [
  {
    ...
    notes: "意味自体は表現できるが、xlsx Blocker記載の同秒0秒visit tie bugが未修正のまま——修正確認が本PRの外（Summary判断#9）。",
    blockerKinds: ["known_bug"],
    optimizationRisk: "LOW",
  },
  {
    no: 7,
    ...
    notes: "facts[].becameLastAtでday-group可能——ただし同秒tie bugが同じsourceに影響。",
    blockerKinds: ["known_bug"],
    optimizationRisk: "LOW",
  },
  {
    ...
    notes: "No.4と同じarea taxonomy欠如。tie bugも重なる。",
    blockerKinds: ["missing_derived_source", "known_bug"],
    optimizationRisk: "LOW",
  },
  {
    no: 9,
    ...
    missingCapabilities: [],
    evidence: [{ file: VC_SOURCES_FILE, symbol: "VcLastOccupantSourcePayload.facts" }],
    notes: "distinct channelId数・複数期間はfacts[]のtimestamp/channelIdからruleが導出可能。tie bugのみが残課題。",
    blockerKinds: ["known_bug"],
    optimizationRisk: "LOW",
  },
];
...
const THEME_14: ManualReadinessEntry[] = [
  {
    ...
    missingCapabilities: ["completed/settled gameを安全に証明するsource semantics"],
    evidence: [
      { file: CASINO_FILE, symbol: "computeCasinoActivityDays" },
      { file: "apps/bot/src/casino/pvp-accept.ts", symbol: "collectAndStartFunded (writer fires before deps.runners[game](...) executes)" },
    ],
    notes:
      "PR #164レビュー§BLOCKER1で修正: casino_activity_daysが証明するのはsuccessful funded participation commitmentであってcompleted gameではない。solo 7種目はsettleSolo()後に書くためcompletion=participationが成立するが、PVP（pvp-accept.tsのcollectAndStartFunded、bj-duel/chinchiro-duel/sashi/indian/poker-duel/chohan-multi）は全経路でcollectStakes成功直後・runner実行前にwriterが発火し、その後runnerが例外/中断してもfactは残る（chohan-multiの「🎴 中断」embedがcode上明示）。activityKeyだけでは書き込みがsolo/PVPどちらの経路かruleから区別できないため、候補全体としては「正常精算まで完了」を保証できない。",
    blockerKinds: ["source_semantic_mismatch"],
    optimizationRisk: "MANAGED",
  },
  {
    ...
    missingCapabilities: ["completed/settled gameを安全に証明するsource semantics"],
    evidence: [
      { file: VC_SOURCES_FILE, symbol: "CasinoActivityDaysSourcePayload.activityDays" },
      { file: "apps/bot/src/casino/pvp-accept.ts", symbol: "collectAndStartFunded (writer fires before deps.runners[game](...) executes)" },
    ],
    notes: "No.66と同じ理由——distinct game family数自体はactivityDays[].activityKeyから数えられるが、各活動が実際に「正常完了」したかはPVP経路では保証されない。",
    blockerKinds: ["source_semantic_mismatch"],
    optimizationRisk: "MANAGED",
  },
  {
    ...
    notes:
      "No.66/67と一括変更しない（PR #164レビュー§BLOCKER1の指示どおり独立判定）。semanticSpecは「幅広いgame familyを複数日に利用する」——「正常完了する」ではなく「利用する」であり、completion保証を要求しない。successful funded participation commitment（casino_activity_daysが実際に証明する範囲）だけで意味を満たせる。",
    blockerKinds: ["none"],
    optimizationRisk: "MANAGED",
  },
  {
    ...
    usableSources: [],
    specializedResolvers: [],
    missingCapabilities: ["第I期core game family一覧のmanifest", "completed/settled gameを安全に証明するsource semantics"],
    evidence: [
      { file: "packages/core/src/casino/participation-history.ts", symbol: "CASINO_ACTIVITY_KEYS" },
      { file: "apps/bot/src/casino/pvp-accept.ts", symbol: "collectAndStartFunded (writer fires before deps.runners[game](...) executes)" },
    ],
    notes:
      "「core」に含めるfamily一覧そのものが未定義（本PRでmanifestを作らない、§17）。PR #164レビュー§BLOCKER1指摘: manifestが後日完成してもNo.66/67と同じcompletion proof不足が残るため、missing_manifestだけでは不十分——「全familyを正常完了」を証明するには両方の欠如を解消する必要がある。",
    blockerKinds: ["missing_manifest", "source_semantic_mismatch"],
    optimizationRisk: "MANAGED",
  },
  {
    no: 70,
