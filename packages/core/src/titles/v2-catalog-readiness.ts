...
const THEME_17: ManualReadinessEntry[] = [
  ...
  })),
  {
    no: 90,
    status: "READY",
    usableSources: ["castle_role_context_safe"],
    specializedResolvers: ["computeCastleRoleContextSafe"],
    missingCapabilities: [],
    evidence: [
      { file: "packages/core/src/titles/v2-castle-role-domain-manifest.ts", symbol: "CASTLE_ROLE_DOMAIN_EDITION_I_MANIFEST" },
      { file: "packages/core/src/role-family/domain-temporal.ts", symbol: "loadTrustedEligiblePublicRoleContexts" },
      { file: "packages/core/src/titles/v2-castle-role-context.ts", symbol: "computeCastleRoleContextSafe" },
    ],
    notes:
      "同一revision/familyのpublic_department+bridge tagだけをeligibleにし、6 normal familyのsubject-active exact occurrenceをrole intervalへ先にJOIN。insideとoutside双方、role-held family breadth、JST日を保持するため『担当内外を含む』を縮小しない。",
    blockerKinds: ["none"],
    optimizationRisk: "MANAGED",
  },
  {
    ...
    notes:
      "activity時点でactiveな全eligible roleのassigned familyをunionし、outside normal activityのfamily/dayを保持する。同一outside familyの複数JST日反復を判定でき、複数outside familyは要求しない。",
    blockerKinds: ["none"],
    optimizationRisk: "MANAGED",
  },
];
