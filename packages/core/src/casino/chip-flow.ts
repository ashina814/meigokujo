...
export class CasinoChipFlow {
  constructor(
    ...
    this.ensureColumn("casino_chip_external_confirmations", "chip_amount", "INTEGER NOT NULL DEFAULT 0");
  }
  ...
  ensureFreeChips(userId: string, required: number, operationId: string): AutoDepositResult {
    ...
    return this.chips.runGroup(
      { groupKey: `chip:auto-deposit:${userId}:${operationId}`, kind: "auto_deposit", actorId: userId },
      () => {
        const freeBefore = this.chips.freeChips(userId);
        const deposited = Math.max(0, required - freeBefore);
        if (deposited) {
          this.chips.deposit(userId, deposited, `chip:auto-deposit:${userId}:${operationId}:land`);
        }
        this.touch(userId);
  ...
  redeemExactFreeChips(
    ...
    return this.chips.runGroup(
      ...
      () => {
        const current = this.chips.freeChips(userId);
        ...
        if (amount) {
          this.chips.redeem(userId, amount, `chip:free-redeem:${userId}:${operationId}:land`);
        }
        this.touch(userId);
  ...
  previewFreeChipRedemption(userId?: string): {
    ...
    const rows = this.listFreeChipUsers()
      .map(({ userId: id }) => ({ userId: id, amount: this.chips.freeChips(id) }))
      ...
      try {
        ...
        result.redeemed.push(
          this.redeemFreeChips(
            row.userId,
            `${prefix}:${row.userId}${cutoffAt == null ? "" : `:${cutoffAt}`}`,
            reason,
          ),
        );
      } catch (error) {
