 ...
 *
 * 敗北倍率を最大2倍に切ると、胴元の収入が落ちて RTP が **+5.73pt** 跳ね上がる
 * （stop>=5 方針）。正本 §1.5 は「基準RTPとの差を ±0.5pt 以内に
 ...
 * ## 実測
 *
 * **各方針 750,000 対局**（250,000 × 3シード・同一サンプルで旧規則と新規則を比較）。
 * 数値の出所は `packages/core/tests/casino-chinchiro-rtp.test.ts` の測定出力ひとつだけで、
 * PR 本文の表も同じ実行結果を写している。
 *
 * | 方針 | 旧RTP | 新RTP | 差 | 調整なし（敗北2倍・エッジ5%のまま） |
 * |---|---|---|---|---|
 * | 目が出たら必ず止める | 87.14% | 87.58% | +0.44pt | 93.23% (+6.09pt) |
 * | スコア4以上で止める | 86.60% | 86.75% | +0.15pt | 92.52% (+5.92pt) |
 * | スコア5以上で止める（マモンと同じ） | 84.14% | 84.18% | +0.04pt | 89.87% (+5.73pt) |
 * | スコア6以上で止める | 81.09% | 80.96% | −0.12pt | 86.54% (+5.45pt) |
 * | 投数まで振り続ける | 74.08% | 73.76% | −0.32pt | 79.01% (+4.93pt) |
 *
 * 全方針で ±0.5pt 以内。方針間のばらつき（約 0.76pt）は定数1つでは吸収できないので、
 * 最大差 0.4pt 台がこの方法の下限になる。
 */
