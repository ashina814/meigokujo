...
export const CASINO_TABLE_CLASSIFICATION: readonly CasinoTableClassification[] = [
  ...
  T({
    ...
    blockerCondition: "none",
    rationale:
      "daily-risk.ts derives a non-financial snapshot from Land and chip ledgers to enforce post-opening loss limits. Pre-opening rows are stale policy metadata and are reset in R6.",
  }),
  T({
    table: "casino_daily_risk_events",
    purpose: "Append-only casino net events used by daily loss-limit enforcement",
    kind: "optional_feature",
    archive: true,
    resetOnApply: true,
    resetPhase: "R6",
    ...
    blockerCondition: "none",
    rationale:
      "daily-risk.ts records replay-safe solo/table net metadata. It is derived from game settlement flow and does not preserve user funds, so pre-opening rows are reset in R6.",
  }),
  T({
    table: "casino_solo_risk_starts",
    purpose: "Solo-game operation to formal-day binding for daily risk settlement",
    kind: "optional_feature",
    archive: true,
    resetOnApply: true,
    resetPhase: "R6",
