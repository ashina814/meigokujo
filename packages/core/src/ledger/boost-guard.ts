...
export function ensureBoostRewardLedgerSchema(db: Database.Database): void {
  ...
  // triggerの定義はコード更新時に必ず置き換える。IF NOT EXISTSだけだと旧定義が残るため。
  db.exec(`
    ...
    CREATE TRIGGER trg_reward_boost_event_required_v1
    BEFORE INSERT ON transactions
    WHEN NEW.type = 'reward_boost' AND NEW.reversal_of IS NULL
    BEGIN
      SELECT CASE WHEN
        NEW.ref_type IS NOT 'discord_boost'
    ...
    CREATE TRIGGER trg_reward_boost_monthly_limit_v2
    BEFORE INSERT ON transactions
    WHEN NEW.type = 'reward_boost' AND NEW.reversal_of IS NULL
    BEGIN
      SELECT CASE WHEN (
